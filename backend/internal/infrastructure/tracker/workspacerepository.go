...
func (r *WorkspaceRepository) Create(ctx context.Context, id uuid.UUID, ownerID uuid.UUID, name string) (*workspacedomain.Workspace, error) {
	transaction, err := r.pool.BeginTx(ctx, pgx.TxOptions{})
	if err != nil {
		return nil, fmt.Errorf("create workspace: begin transaction: %w", err)
	}
