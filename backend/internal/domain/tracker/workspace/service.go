...
func (s *WorkspaceService) Create(ctx context.Context, ownerID uuid.UUID, name string) (*Workspace, error) {
	if name == "" {
		return nil, fmt.Errorf("%w: name is required", ErrInvalidWorkspace)
	}
	w, err := s.repository.Create(ctx, uuid.New(), ownerID, name)
	if err != nil {
