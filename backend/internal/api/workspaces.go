...
func (h *WorkspaceHandler) CreateWorkspace(ctx context.Context, req model.CreateWorkspaceRequestObject) (model.CreateWorkspaceResponseObject, error) {
	...
	workspace, err := workspaceToDomain(userID, *req.Body)
	if err != nil {
		return model.CreateWorkspace400JSONResponse{
			BadRequestJSONResponse: newBadRequest("invalid_input", "invalid workspace"),
		}, nil
	}
	created, err := h.service.Create(ctx, workspace)
	if err != nil {
		return nil, fmt.Errorf("create workspace: %w", err)
	}
	return model.CreateWorkspace201JSONResponse(workspaceFromDomain(created)), nil
}
